MapCloud.GPSDemSlopeDialog = MapCloud.Class(MapCloud.Dialog,{
	...
	registerPanelEvent : function(){
		...
		dialog.panel.find(".btn-choose-input-raster").click(function(){
			MapCloud.raster_db_dialog.showDialog("demSlope",false);
		});

		// choose output sourcename
		dialog.panel.find(".btn-choose-output-source-name").click(function(){
			MapCloud.raster_db_dialog.showDialog("demSlope",true);
		});
		...
		this.panel.find(".gps-btn-oper-btn").click(function(){
			...
			if(!nameReg.test(outputRasterName)){
				MapCloud.notify.showInfo("请输入有效的输出影像名称","Warning");
				dialog.panel.find(".gps-output-raster-name").focus();
				return;
			}
			...
			gpsManager.demSlope(dialog.inputSourceName,dialog.inputRasterName,
				dialog.inputRasterPath,dialog.outputSourceName, outputRasterName,
				dialog.outputRasterPath,dialog.demSlope_callback);
		});
	...
	// 结果
	demSlope_callback : function(result){
		MapCloud.notify.hideLoading();
		var dialog = MapCloud.gps_dem_slope_dialog;
		var outputRasterName = dialog.panel.find(".gps-output-raster-name").val() 
