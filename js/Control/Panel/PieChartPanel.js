MapCloud.PieChartPanel = MapCloud.Class(MapCloud.Panel,{
	// 图
	chartLayer 	: null,
	...
	cleanup : function(){
		...
		this.panel.find(".chart-table-name").val("饼状图");
		this.panel.find(".chart-table-radius").val("40");
	...
	displayChartFields : function(){
		...
		this.panel.find(".form-group-field").remove();
		this.panel.find("#pie_chart_table_style_coll .panel-body .form-horizontal").append(html);
	...
	getChartOption : function(){
		var radius = this.panel.find(".chart-table-radius").val();
	...
	addChartLayer_callback : function(result){
		if(result == "success"){
			mapObj.draw();
			var that = MapCloud.pie_chart_panel;
	...
	setChartLayer : function(chartLayer){
		...
		if(chartOption != null){
			...
			this.panel.find(".chart-table-radius").val(radius);
