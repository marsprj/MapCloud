...
MapCloud.StyleManagerDialog = MapCloud.Class(MapCloud.Dialog,{
		...
		this.panel.find(".quantities-pane .classes").each(function(){
			$(this).change(function(){
				...
				if(dialog.minMaxValue == null){
					...
					// styleManager.getColorMap(id,count,dialog.getColorMapMinMax_callback);
					styleManager.getColorMapNew(id,count,dialog.getColorMapMinMax_callback);
				}
	...
	getColorMaps_callback : function(colorMaps){
		...
		dialog.panel.find(".unique-pane .color-ramp-div ul li").click(function(){
			...
			if(dialog.uniqueValues == null){
				...
				// styleManager.getColorMap(id,count,dialog.getColorMap_callback);
				styleManager.getColorMapNew(id,count,dialog.getColorMap_callback);
			}
		});
		...
		dialog.panel.find(".custom-pane .color-ramp-div ul li").click(function(){
			...
			styleManager.getColorMapNew(id,count,dialog.getColorMapCustom_callback);
		});
	},
	...
	getMinMaxValue_callback : function(obj){
		...
		// 获得色阶值
		var id = dialog.panel.find(".quantities-pane .select-color-ramp li").attr("cid");
		// styleManager.getColorMap(id,classes,dialog.getColorMapMinMax_callback);
		styleManager.getColorMapNew(id,classes,dialog.getColorMapMinMax_callback);
	...
	getStyleByMinMaxValues : function(defaultRule,field,min,max,classes,colorValues){
		...
		for(var i = 0; i < classes; ++i){
			...
			if(type == GeoBeans.Symbolizer.Type.Point){
				...
				var strokeColor = new GeoBeans.Color();
				strokeColor.setByHex(colorValue,strokeOpacity);
				if(symbolizer.stroke == null){
					symbolizer.stroke = new GeoBeans.Stroke();
				}
				symbolizer.stroke.color = strokeColor;
